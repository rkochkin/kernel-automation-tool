...
class KernelBuilder:
    def __init__(self, config: ProjectConfig):
        ...
        self.git_manager = GitManager(
            config.kernel.repository, 
            config.kernel.branch
        )
    ...
    def build(self) -> bool:
        """Main build method"""
        try:
            print(f"Building kernel {self.config.kernel.name} version {self.config.kernel.version}")
    ...
    def _apply_patches(self):
        ...
        for patch_path in self.config.patches:
            ...
            try:
                subprocess.run(
                    ['patch', '-p1', '-d', self.source_dir, '-i', 
                     os.path.abspath(patch_path)],
                    check=True, capture_output=True
                )
            except subprocess.CalledProcessError as e:
                raise RuntimeError(f"Failed to apply patch {patch_path}: {e.stderr.decode()}")
    ...
    def _configure_kernel(self):
        ...
        try:
            ...
            subprocess.run(
                ['make', f'ARCH={self.config.build.arch}', 
                 self.config.build.config_file],
                cwd=self.source_dir, env=env, check=True, capture_output=True
            )
    ...
    def _build_kernel(self):
        ...
        try:
            subprocess.run([
                'make', '-j', str(self.config.environment.make_jobs),
    ...
    def _build_modules(self):
        ...
        try:
            ...
            subprocess.run([
                'make', '-j', str(self.config.environment.make_jobs),
                f'ARCH={self.config.build.arch}', 'modules'
            ], cwd=self.source_dir, env=env, check=True, capture_output=True)
            ...
            subprocess.run([
                'make', f'ARCH={self.config.build.arch}',
                f'INSTALL_MOD_PATH={modules_path}', 'modules_install'
            ], cwd=self.source_dir, env=env, check=True, capture_output=True)
